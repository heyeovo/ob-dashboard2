...
- 滚动 revision 对齐时，已有 Haven turn ID 或原生 user UUID 是权威关联，不再被展示正文差异否决。仅对已有滚动窗口中“无 Haven 关联、无 assistant 消息、无工具链、且不对应 Haven 空回复成功轮次”的 user-only 失败半截输入做隔离；固定→滚动首次迁移不得据此跳过来源。其余冲突继续 fail closed，报错只输出类别和数量，不输出聊天正文。
