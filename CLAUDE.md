...
| `app/api/cc-context-audit/` | 滚动窗口额外只读预检完整 transcript 对 Haven 轮次的可对齐性，报告被隔离的仅用户输入半截记录、失败类别及无正文候选记录的序号/UUID/主动唤醒标记（不返回诊断正文）；不生成新版本、不修改设置或会话指针。 |
...
- 滚动 revision 对齐时，已有 Haven turn ID 或原生 user UUID 是权威关联，不再被展示正文差异否决。SDK/CLI 在工具回合后注入的固定“无可见输出请续写”提示，以及中断后固定的标记/续写提示，只在有前一真实轮次可锚定时归入该轮 envelope；原始 transcript 消息、工具顺序和正文不改写，孤立提示仍按异常处理。仅对已有滚动窗口中“无 Haven 关联、无 assistant 消息、无工具链、且不对应 Haven 空回复成功轮次”的 user-only 失败半截输入做隔离；固定→滚动首次迁移不得据此跳过来源。其余冲突继续 fail closed，报错只输出类别和数量，不输出聊天正文。
