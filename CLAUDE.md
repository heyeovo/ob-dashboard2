...
| `cc-chat-selfhost/route.ts` | 自建纯聊天入口：服务端读取 Haven 配置/历史，按 cc 同一格式注入本轮隐藏北京时间，直连 Anthropic-compatible SSE，记录上游生成耗时/速度，严格写回成功后才完成 |
