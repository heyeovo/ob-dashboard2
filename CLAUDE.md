...
- 每个已保存 user/assistant 消息使用 Haven 分配的永久 `msg_*` ID，并保存按窗口日界线计算的 `chat_day`。滚动配置、revision 和保存时的 turn watermark 均持久化在 Haven；CC 的原生 resume key 必须包含 revision，禁止新配置续接旧 revision。滚动原文不再序列化进 `rolling_window_context`：CC 冷启动先用 Agent SDK `SessionStore` 物化原生 transcript，再从该 transcript resume；普通轮保持真实 `user/assistant`，Haven 中 assistant-only 的 `agent_wake` 还原为隐藏 `<agent_wake/>` 输入再接原 assistant 消息；selfhost 直接以原生 messages 重建。日回顾、钉选桶和日记仍属于背景 Context。SDK 不允许 `sessionStore` 与文件 checkpoint 同开，因此 store-backed 滚动 CC 关闭 `enableFileCheckpointing`，固定窗口仍保留文件回退。同 revision 跨进程从持久 RollingSeedStore 恢复；revision 变化时按完整外部 user 轮次对齐 Haven 永久 turn，只删除退出 raw 的整轮并原样保留其余轮次中的动态召回、`tool_use`、`tool_result` 与顺序。Haven 同时记录上一 revision 的策略和日期模式；只有明确的 fixed→rolling 首次建种或上一版 review/omit→本版 raw 的日期允许 Haven 正文恢复。daily→daily、迁移类型未知、旧 session/持久 transcript 缺失、轮次无法唯一对应，或任一 raw→raw 轮次缺失时均 fail closed；成功重建的来源、条数、工具/召回数量与正文恢复数随首轮写入 Haven，审计页可长期复核。设置页明确提示重新加入日期的降级。选为原文的可见日期只排除这些日期内已召回/新建的桶；日回顾或不带日期退出排除集合。钉选桶每轮从 Haven 读取最新正文，不冻结进 handoff。
