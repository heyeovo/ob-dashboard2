...
| `app/workbench/` | 工作台；“调参”下提供默认收起的本轮上下文审计，按当前会话只读展示滚动原文、背景拼接、SDK SessionStore 实际落盘消息、最近实际与当前/下一轮 Dashboard system prompt，以及最近一轮 token/cache 报告，不触发额外模型请求 |
