...
- 接入 `system/status: compacting`，自动压缩进行中时顶栏显示“Context 压缩中”，结束状态不额外请求模型。
- 顶栏新增 CC 常驻 Context 胶囊；80% 起提示准备换窗，90% 起建议本轮结束后换窗。点击打开“本窗口设置”。
- “本窗口设置”显示当前/上次模型调用 Context、百分比、约剩 token、模型切换待确认、最近压缩及次数。Prompt cache 继续使用原有两档 TTL，字段与展示均独立。
...
新增测试覆盖：多次模型请求只保留最后一次 Context、`iterations` 最后一项、累计 usage 不覆盖 Context、自动压缩事件顺序与 raw 持久化、手动分隔线历史位置、SSE 新事件分发、Pro/API 原生 session 隔离。

## 7. 用户验收步骤

1. CC 工作模式发一轮无工具回复，确认顶部出现 `Context x/200k`（Pro Opus 4.6）且消息 token 面板标题为“本轮累计消耗”。
2. 发一轮含多个工具的任务，确认顶部 Context 不会按工具次数成倍膨胀。
3. 打开“本窗”，确认 Context 状态位于“正在用”和 Prompt cache 附近，Prompt cache 倒计时仍独立。
4. 在线且空闲时点“立即压缩”；确认弹出成本提示，并在消息间出现“手动压缩已完成 · Nk → Nk”。历史不足时应明确显示没有发生压缩。
5. 若真实发生自动压缩，确认分隔线出现在当轮真实位置，顶部数字切到 post tokens，刷新后仍能恢复。
6. 切换 Pro/API，确认两条线路不串 Context。
