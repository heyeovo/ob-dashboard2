...
- 下一步先处理用户发现的阶段一功能 bug（另开窗口、先描述现象并定点调查），再决定是否提交/部署阶段二。阶段二部署后才进行少量真实日期人工检查；首次历史回填仍必须先展示额度估算并获得用户明确确认。
...
下一个窗口先单独处理用户发现的阶段一 bug，不把 bug 修复混入本阶段二提交；现象和复现步骤尚未提供，不能在 handoff 中猜测。bug 处理完后再提交/部署阶段二，并用少量真实日期人工检查风格、事实、情绪和切片边界。
