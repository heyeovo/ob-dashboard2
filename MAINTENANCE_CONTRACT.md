...
| 删了东西 / 有遗留 / 有不确定项 | `TECH_DEBT.md` | 见下节固定收尾流程 |
...
| 未接入功能新增 / 完成 | `TECH_DEBT.md`「未接入功能」节 |
...
5. **有遗留 / 未定 / 没做完的** → 写进 `TECH_DEBT.md`，不口头带过。
6. **给用户建议 commit message**（用户自己 push）。
7. **改了 Haven 后端** → 提醒重新部署 Zeabur（代码改动不会自动生效）。
